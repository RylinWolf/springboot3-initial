...
项目提供用户模块（包括头像上传与 OSS 日志记录模块）、权限验证模块。
...
- MyBatis Flex
- knife4j
- Apache Commons Validator：用于验证工具类的邮箱验证等节点
- Apache Commons Imaging：用于验证图片格式
...
若你不了解什么是 OSS，可通过 <a href="https://help.aliyun.com/zh/oss/">阿里云 OSS 文档</a>  了解关于 OSS 对象存储的内容。

本项目内置头像上传功能。该功能依赖于 OSS 模块，需要正确配置参数后才可使用。

在 `application-local.yaml`（或者你自定义的配置文件）中，配置以下参数:
...
custom:
  oss:
    aliyun:
      endpoint: # oss 接入点
      region: # oss 地区
      avatar-bucket-name: # 用于存放头像的桶的名称
```

在系统环境变量中，配置 AK。（本项目基于 AK 进行身份验证）
...
详细配置方式参考 <a href="https://help.aliyun.com/zh/oss/user-guide/oss-sdk-quick-start?spm=a2c4g.11186623.help-menu-31815.d_0_0_3_4.21f073683JAvWh#c54ab56b370r7"> SDK 快速入门 </a> 。



### 业务流程

为保障服务器安全性，修改用户功能分为上传头像和更新用户两个部分。

若要修改头像，则需先发起上传头像请求，头像上传结束后会将上传地址根据用户进行缓存，该缓存具有有效期，需要在有效期内使用。

在更新用户时，若要删除头像，则将头像字段设为 null，否则设为任意值，后端会检查是否有缓存的头像地址，并进行决策。

后端根据指纹进行解析，获取对应的头像地址并自动注入。

```mermaid
flowchart LR
    前端请求 --> request

    request[修改用户] --> avatar[上传头像]
    avatar --> req_avatar(上传头像请求)
    req_avatar --> upload(执行上传)
    request --> update[更新用户]
    update --> update_avatar(更新用户头像)
    update --|同步| --> update_info(更新用户信息)
    update_avatar --> get_avatar{检查是否上传头像}

    get_avatar --上传 --> success[后端更新]
    get_avatar --未上传 --> failed[忽略]
...
**需要注意的是**，头像上传模块目前没有做文件缓存等优化，用户即使上传相同的文件，也会作为不同的文件存储在 oss 中。

考虑到本项目已经通过 Spring 配置、图像校验器等方式限制了图片大小，所以兼顾到复杂性，后续也不会试图为头像服务添加基于包括但不限于计算 md5 实现的文件防重复机制。

但是照顾到存储空间的问题，本项目设计了清除未使用头像的功能。

清除未使用头像，构思了两种方法：

- 通过上传记录清除：本项目中内置了 OSS 上传日志模块，通过上传日志表可以查询指定用户的历史上传记录，并根据存储地址操作 OSS 删除。
  - 较为轻量，但有效
  - 若日志模块不可靠，则可能存在残留
- 通过是否使用删除：获取所有用户正在使用的头像，与 OSS 进行比对。删除所有未在使用的。
  - 清除彻底，只会保留正在使用的头像
  - **需要考虑并发问题**：执行删除的过程中，若用户上传了头像但尚未更新，则可能会被删除
  - 执行效率低
...
- 定时任务
  ...
  - 定时时间较难把握
    ...
    - 若定时时间较长，可能某用户会产生多个头像记录
    - 定时时间过短，若这个范围内无用户进行头像操作，则浪费性能
...
综合考虑后，本项目选择定时任务 + 在上传头像后进行头像清理，使用 缓存 + 定时任务 的方式执行清除。理由如下：

1. 头像文件已经做了大小限制，多存储一条可以接收。再通过定时任务进行扫描，把多余的数据进行清除。
2. 项目已经封装了 Redis 工具类，可以方便地缓存信息。调用 OSS 工具耗时较长，适合放在定时任务中处理，便于管理的同时减少上传头像的响应时间

#### 执行步骤

1. 为 OSS 日志表新增 “删除” 字段，标记文件是否已删除，添加索引
2. 调用服务，获取操作用户的未删除的文件记录
3. 保留用户正在使用的头像，将其他记录保存在 Redis 中
4. 设置两个定时任务：
   1. 扫描日志，按照用户与删除状态分组，若有未删除数据则获取数据、放入 Redis 并标记为删除
   2. 从缓存中获取记录，调用 OSS 工具执行删除
...
为方便监控用户上传头像的记录，项目提供了 OSS 上传日志记录功能。

若不使用 OSS 功能或不想使用该模块，该模块可较为简单地移除。
